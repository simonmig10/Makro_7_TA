...
1. Husholdninger maximere deres budget restriktion (eleminere alle under F)
2. Økonomiens kapital beholdning kan ikke være negativ (eleminere alle over F)
...
```{r, echo=FALSE}
knitr::include_graphics(rep("f(x).png", 1), dpi = 100)
```

## Maclaurin polynomials (udledning)

```{r, echo=FALSE}
knitr::include_graphics(rep("f(0).png", 1), dpi = 100)
```
...
$$
\begin{aligned}
p(x)= f(0) + f'(c)(x-c) + f''(c)*\frac{1}{2!}(x-c)^2\\ + f'''(c)*\frac{1}{3!}(x-c)^3 + ... + f^n(c)*\frac{1}{n!}(x-c)^n
\end{aligned}
$$
...
```{r, echo=FALSE}
knitr::include_graphics(rep("f(x)c.png", 1), dpi = 150)
```
...
```{r, echo=FALSE}
knitr::include_graphics(rep("f(c).png", 1), dpi = 150)
```
...
Vi har derfor nu udviklingen i forskellen mellem $c$ og $c^*$ samt $k$ og $k^*$ 
...
Resten af slides kan gennemgå selv, men er ikke relevant for jeres eksamens opgaver. 
